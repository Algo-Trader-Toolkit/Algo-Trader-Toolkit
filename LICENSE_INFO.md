# License Information

## Commercial License

**Algo-Trader's Toolkit** is a commercial product. All rights reserved.

### What You Get

When you purchase any tier, you receive:

- ✅ **Full source code** (Python)
- ✅ **Lifetime access** to your purchased tier
- ✅ **One-time payment** — no subscriptions
- ✅ **Free version updates**
- ✅ **Personal & commercial use** for your own trading
- ✅ **Unlimited modifications** for your own use
- ✅ **Email support** for setup questions
- ✅ **Enterprise tier:** Priority support + onboarding assistance

### What's NOT Allowed

- ❌ Redistribution of source code
- ❌ Reselling or sublicensing
- ❌ Sharing with others (each user needs their own license)
- ❌ Using in a commercial service for others

### License Enforcement

All copies are watermarked with buyer information. Unauthorized distribution will result in:
- DMCA takedown notices
- Legal action under copyright law
- Permanent blacklist from future products

---

## Purchase

| Tier | Price | Best For |
|------|-------|----------|
| Standard | $49 | Fast strategy testing with 12+ indicators & Fibonacci levels |
| Pro | $149 | Automating technical & pattern-based trading workflows |
| Ultimate | $299 | Quant-level research, ML validation, and multi-factor confirmation |
| Enterprise | $749 | Fully unattended live trading with real-time safety gates |

| Tier | What's Included |
|------|------------------|
| **Standard** | 12+ indicators, Fibonacci levels, realistic backtesting |
| **Pro** | *Everything in Standard* + Pattern recognition + chart patterns + candlestick analysis |
| **Ultimate** | *Everything in Pro* + ML pipeline + Elliott Wave + FVG + intelligent exits |
| **Enterprise** | *Everything in Ultimate* + **LIVE TRADING** + safety monitors + Telegram alerts |

👉 **[Buy on Gumroad](https://algotoolkitt.gumroad.com/)**

---

## Refund Policy

Refunds are evaluated case-by-case for technical access issues, not performance guarantees.

---

## Questions?

Contact: **algo.toolkitt@gmail.com**

⚠️ **All licensed distributions contain unique watermark signatures. Unauthorized resale or sharing can be traced and prosecuted.**
